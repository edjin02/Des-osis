    ...
    ngOnInit(): void {
      ...
      this.adminService.getGradeLevelListing().subscribe((grades: any) => {
        this.gradeLevels = grades.map((e: any) => {
          return { ...e, isChecked: false, disabled: true }; // Set isChecked to false for all checkboxes
        });
      ...
      this.subjectData.forEach((subject: any) => {
        subject.gradeLevels.forEach((gradeLevel: any, index: number) => {
          gradeLevel.isChecked = false; // Uncheck all checkboxes
          gradeLevel.disabled = true; // Disable all checkboxes
        });
      });
    }
