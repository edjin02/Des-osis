...
export class RoomAssignComponent implements OnInit {
  ...
  deleteMe(grId:any,clId:any){
    let classDeact:any = this.classView;
    let dialogRef = this.modal.open(ConfirmDialogComponent,{
      width: '300px',
      disableClose: true,
      data: {},
    });
    dialogRef.afterClosed().subscribe((pl) => {
      if (pl === true) {
        classDeact.active = false;
        this.adminService.nSaveClass(classDeact.id, classDeact).then(()=>{
          console.log('saved');
          this.modal.closeAll();
        })
      }
