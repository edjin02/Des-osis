...
export class ScheduleFormComponent implements OnInit {
  ...
  selectTemplate(e:any){
    ...
    this.adminService.nGetSchedTempById(this.selectedTemplate).subscribe((res:any)=>{
      ...
      this.pushSubject(this.subjects)
    })
  }
  getDetails() {
  ...
  pushSubject(arr:any){
    let j = []
    for (let dayIn = 0; dayIn < 5; dayIn++) {
      let i = [];
      for (let pushIn = 0; pushIn < this.schedRow.length; pushIn++) {
        i.push(arr);
      }
      j.push(i);
    }
  ...
  getId(dayIt:any,index:any,data:any,ev:any = null){
    // var subIn = this.subjects.findIndex(function(subject:any){
    //   return subject.id == ev.target.value
    // });
    for (let scIn = 0; scIn < this.schedRow.length; scIn++) {
      if(scIn!==index){
        let nSubject = this.subjectDay[dayIt][scIn].filter((obj:any)=> !obj.id.includes(ev.target.value));
        this.subjectDay[dayIt][scIn] = nSubject;
      }
    }
  }
